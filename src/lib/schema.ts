...
export const DATASETS: Record<DatasetKey, DatasetSpec> = {
  holdings: {
    ...
    domain: 'investments',
    blurb:
      'One row per position, valued on a single date. Each import becomes a versioned snapshot, so re-importing next month builds a return series rather than overwriting.',
    fields: [
      { key: 'ticker', label: 'Ticker / symbol', type: 'string', required: true, synonyms: ['ticker', 'symbol', 'code', 'stock'] },
      { key: 'name', label: 'Name', type: 'string', synonyms: ['name', 'security', 'description', 'asset', 'fund'] },
      { key: 'quantity', label: 'Quantity', type: 'number', required: true, synonyms: ['quantity', 'qty', 'shares', 'units', 'holdings'] },
      { key: 'price', label: 'Price per unit', type: 'number', synonyms: ['price', 'market price', 'nav', 'last', 'close', 'unit price'] },
      { key: 'value', label: 'Market value', type: 'number', synonyms: ['market value', 'value', 'amount', 'current value', 'balance'], hint: 'If absent, computed as quantity × price.' },
      { key: 'costBasis', label: 'Total cost basis', type: 'number', synonyms: ['cost', 'cost basis', 'book value', 'invested', 'purchase value', 'acquisition'] },
      { key: 'assetClass', label: 'Asset class', type: 'enum', synonyms: ['asset class', 'class', 'category', 'type', 'sector'], default: 'Unclassified' },
      { key: 'geography', label: 'Geography', type: 'string', synonyms: ['geography', 'region', 'country', 'market', 'location'], default: 'Unspecified' },
      { key: 'currency', label: 'Currency', type: 'currency', synonyms: ['currency', 'ccy', 'curr'], default: 'PHP' },
      { key: 'account', label: 'Account / broker', type: 'string', synonyms: ['account', 'broker', 'platform', 'custodian', 'wallet'], default: 'Default' },
    ],
  },
  ...
  bookings: {
    ...
    fields: [
      ...
      { key: 'netRevenue', label: 'Net payout', type: 'number', synonyms: ['net', 'payout', 'earnings', 'net earnings', 'net amount'], hint: 'If absent, computed as gross − fees.' },
